...
{
	//la variable estatica en este caso es un array para los 4096 fd que se pueden manejar al mismo tiempo
	static char	*memory[4096];
	char		*return_line;

	//se añade la condicion que el fd sea mayor a 4096 para el error
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > 4096)
		return (0);
	//todas las funciones operan con la posición del array requerida para cada fd
	memory[fd] = ft_memory_read(fd, memory[fd]);
	if (!memory[fd])
		return (0);
	//return_line no es un array porque es solo una variable que se sobreescribe la memoria una vez se usa
